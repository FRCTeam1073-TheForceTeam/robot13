...
Shooter::Shooter() : Subsystem("Shooter") {
	...
	primaryJag->ChangeControlMode(CANJaguar::kSpeed);
	primaryJag->SetSpeedReference(CANJaguar::kSpeedRef_Encoder);
	primaryJag->ConfigEncoderCodesPerRev(360);	//maybe 360
	primaryJag->EnableControl();
	
	supportJag->ChangeControlMode(CANJaguar::kSpeed);
	supportJag->SetSpeedReference(CANJaguar::kSpeedRef_Encoder);
	supportJag->ConfigEncoderCodesPerRev(360);	//maybe 360
	supportJag->EnableControl();
...
void Shooter::ShooterOnOff(bool on){
	...
	isShooterMotorOn = on;
	if(on){
		printf("Primary Jag:\t%d", speed);
		printf("Support Jag:\t%d", GetSupportSetSpeed());
...
void Shooter::SetPID(double P, double I, double D){
	printf("Setting P:\t%f\nI:\t%f\nD:\t%f\n", P, I, D);
	primaryJag->SetPID(P, I, D);
	supportJag->SetPID(P, I, D);
}
